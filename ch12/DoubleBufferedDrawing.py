...
class RadarGraph(wx.Window):
    '''A simple radar graph that plots a collection of values in the range of 0-100 onto a polar coordinate system designed to easily show outliers,etc.
    You might use this kind of graph to monitor some sort of resource allocation metrics, and a quick glance at the graph can tell you when conditions 
    are good(within some accepted tolerance level) or approaching critical levels (total resource consumption).'''
    ...
    def OnPaint(self, vet):
        # This automatically Blits self.buffer to a wx.PaintDC when the dc is destroyed, and so nothing else needs done.
        dc = wx.BufferedPaintDC(self, self.buffer) # 1 Use the buffer context to refresh the window

    def InitBuffer(self): # 2 create buffer
        # Create the buffer bitmap to be the same size as the window, then draw graph to it.Since we use wx.BufferedDC whatever is drawn to the buffer 
        # is also drawn to the window.
    ...
    def SetData(self, newData):
        ...
        # since the data changed, we must update the buffer and window
        dc = wx.BufferedDC(wx.ClientDC(self), self.buffer)
        self.DrawGraph(dc)
    ...
    def DrawGraph(self, dc): # draw the graph
        ...
        dc.DrawText(self.title, (dw - tw)/2, spacer) # draw the title
        ...
        #calculate a scale factor to use for drawing the graph based on the minimum available width or height
        ...
        # Now find the coordinates for each data point, draw the labels, and find the max data point
        ...
        for i, label in enumerate(self.labels):
            val = self.data[i]
            point = self.PolarToCartesian(val * scale, angle, cx, cy) # convert data to cordination point
